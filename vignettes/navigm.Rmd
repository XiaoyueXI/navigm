...
$$
\begin{aligned}
\mathbf{y}_1, ..., \mathbf{y}_n &\overset{\text{iid}}{\sim} \mathcal{N}_P(\mathbf{0}, \boldsymbol{\Omega}^{-1}), \quad n = 1, ..., N, \quad \boldsymbol{\Omega} \in \mathcal{M}^+,  \\
\omega_{ij}\mid \delta_{ij}, \tau &\sim \delta_{ij}\mathcal{N}\left(0, \nu_1^2\tau^{-1}\right) + (1-\delta_{ij}) \mathcal{N}\left(0, \nu_0^2\tau^{-1}\right),\quad  \nu_0 \ll \nu_1,\quad 1 \leq i<j \leq P,\\
...
\beta_q \mid \gamma_q, \sigma^2 &\sim  \gamma_{q}\mathcal{N}(0,\sigma^2) + (1-\gamma_{q}) \delta(\beta_q) , \quad q=1,\ldots, Q, \quad \gamma_q\mid o \sim \text{Bernoulli}(o), \quad \\
\zeta &\sim \mathcal{N}(n_0, t_0^2), \quad \sigma^{-2} \sim \text{Gamma}(a_\sigma, b_\sigma), \quad o \sim  \text{Beta}(a_o, b_o),
\end{aligned}
$$
...
$$
\begin{aligned}
\beta_q \mid \sigma^2 &\sim  \mathcal{N}(0,\sigma^2) , \quad q=1,\ldots, Q,\\ 
...
* ``version = 2``: $$\Phi(\rho) \sim \mathcal{N}(n_0, t_0^2), $$ which is more comparable with our extensions. 
