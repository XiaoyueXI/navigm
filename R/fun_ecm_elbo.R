...
get_elbo_gm_ecm_v1 <- function(Omega,
                               rho,
                               tau1,
                               P1,
                               E1,
  ...
  N * determinant(Omega, logarithm = TRUE)$modulus[1] / 2 -
    ...
    log(v1) * sum(P1[bool_up]) -
    log(v0) * sum(1 - P1[bool_up])  +
    #
    (P * (P - 1) / 2) * (log(tau1 + eps) / 2) -
    tau1 * sum(Omega[bool_up] ^ 2 * E1[bool_up]) / 2 +
    (a_tau - 1) * log(tau1 + eps) - b_tau * tau1 +
    #
    (sum(P1[bool_up]) + a_rho - 1) * log(rho + eps) +
    (sum(1 - P1[bool_up]) + b_rho - 1) * log(1 - rho + eps)
...
get_elbo_gm_ecm_v2 <- function(Omega,
                               zeta,
                               tau1,
                               P1,
                               E1,
...
get_elbo_gmn_ecm <- function(Omega,
                             zeta,
                             beta,
                             tau1,
                             tau2,
...
get_elbo_gmss_ecm <- function(Omega,
                              zeta,
                              beta,
                              o,
                              tau1,
