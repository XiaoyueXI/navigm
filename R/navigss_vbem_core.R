...
#' Node-level auxiliary variable informed Gaussian spike-and-slab using variational EM.
#'
...
#' @param list_init A list containing the initial variational parameters:
...
#' @details \code{navigss_vbem_core} implements a Gaussian graphical model
...
#' res_navigss_vbem_core <-navigss_vbem_core(Y, V, transformV = F)
#' # res_navigss_vbem_core <-navigss_vbem_core(Y, V, method = 'GMN', transformV = F))
#' # res_navigss_vbem_core <-navigss_vbem_core(Y, V, method = 'GM', version = 1, transformV = F))
#' # res_navigss_vbem_core <-navigss_vbem_core(Y, V, method = 'GM', version = 2, transformV = F))
#'
#' @import matrixcalc
#' @export
#'
